...
namespace 扑克牌
{
    class MainClass
    {
        public static void Main(string[] args) {
            ...
            for (int i = 1; i <= 52; i++) { poke.Add(i); }
            List<int> newPoke = ListRandom(poke); int[] user1 = new int[13];
            int[] user2 = new int[13]; int[] user3 = new int[13];
            int[] user4 = new int[13]; for (int i = 0; i < 13; i++)
            { user1[i] = poke[0]; poke.Remove(poke[0]); }
            for (int i = 0; i < 13; i++) { user2[i] = poke[0]; poke.Remove(poke[0]); }
            for (int i = 0; i < 13; i++) { user3[i] = poke[0]; poke.Remove(poke[0]); }
            for (int i = 0; i < 13; i++) { user4[i] = poke[0]; poke.Remove(poke[0]); }
            Console.WriteLine("用户一的牌："); Console.WriteLine("");
            var u1 = user1.OrderByDescending(i => i);
            foreach (int x in u1) { string name = Name(x); Console.Write(name + "  ,"); }
            Console.WriteLine(""); Console.WriteLine(""); Console.WriteLine("用户二的牌：");
            Console.WriteLine(""); var u2 = user2.OrderByDescending(i => i);
            foreach (int x in u2) { string name = Name(x); Console.Write(name + "  ,"); }
            Console.WriteLine(""); Console.WriteLine(""); Console.WriteLine("用户三的牌：");
            Console.WriteLine(""); var u3 = user3.OrderByDescending(i => i);
            foreach (int x in u3)
            { string name = Name(x);
           Console.Write(name + "  ,"); }
            Console.WriteLine("");
            ...
            Console.WriteLine(""); var u4 = user4.OrderByDescending(i => i);
            foreach (int x in u4) { string name = Name(x); Console.Write(name + "  ,"); } }
        public static string Name(int id)
        {
            string Name = "";
            switch (id) { case 1: Name = "方块3"; break;
        ...
        static List<int> ListRandom(List<int> myList)
        {
            ...
            { index = ran.Next(0, myList.Count - 1);
                if (index != i)
                {
                    temp = myList[i]; myList[i] = myList[index]; myList[index] = temp;
                }
            }
            return myList; }
    }
